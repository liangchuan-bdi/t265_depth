...
t265Depth::t265Depth(ros::NodeHandle &node, ros::NodeHandle &private_node)
  ...
  private_node.param("speckle_window_size", speckle_window_size_,
                     kSpeckleWindowSize);
  private_node.param("sad_window_size", sad_window_size_, kSADWindowSize);
  // bm parameters
  private_node.param("texture_threshold", texture_threshold_,
                     kTextureThreshold);
  ...
  pub_camera_info_left_ = node.advertise<sensor_msgs::CameraInfo>(
      input_topic_left_ + "/rectified" + "/info", 10);
  pub_camera_info_right_ = node.advertise<sensor_msgs::CameraInfo>(
  ...
  sync_ptr->registerCallback(
      boost::bind(&t265Depth::syncCallback, this, _1, _2));
}

t265Depth::~t265Depth() {}

void t265Depth::syncCallback(
    const sensor_msgs::Image::ConstPtr &image_msg_left,
  ...
  if (scale_ < 1) {
    resize(image_left_, image_left_,
           Size(image_left_.cols * scale_, image_left_.rows * scale_));
    resize(image_right_, image_right_,
           Size(image_right_.cols * scale_, image_right_.rows * scale_));
  }
  elaborateImages(header_out);
  try {
    out_img_msg_left =
        cv_bridge::CvImage(std_msgs::Header(), "mono8", undist_image_left_)
            .toImageMsg();
    out_img_msg_right =
        cv_bridge::CvImage(std_msgs::Header(), "mono8", undist_image_right_)
            .toImageMsg();
...
void t265Depth::publishCameraInfo(cv::Mat K1, cv::Mat K2, cv::Mat P1,
                                  cv::Mat P2, cv::Mat R1, cv::Mat R2) {
...
void t265Depth::initializeRectificationMapping(std::string param_file_path) {
  ...
  cv::FileStorage param_file =
      cv::FileStorage(param_file_path, cv::FileStorage::READ);
  ...
  // The resolution of the output rectified images. Lower resolution images
  ...
  float stereo_fov_rad_x = 90 * (M_PI / 180);  // 90 degree desired fov
  float stereo_fov_rad_y = 55 * (M_PI / 180);  // 90 degree desired fov
  ...
  float stereo_focal_px = stereo_width_px / 2 / tan(stereo_fov_rad_x / 2);
  float stereo_focal_py = stereo_height_px / 2 / tan(stereo_fov_rad_y / 2);
  ...
  cout << "stereo_focal_px: " << stereo_focal_px << "\n"
       << "stereo_focal_py: " << stereo_focal_py << "\n"
  ...
  P_left << stereo_focal_px, 0, stereo_cx, 0, 0, stereo_focal_py, stereo_cy, 0,
      0, 0, 1, 0;
  ...
  Q = (cv::Mat_<float>(4, 4) << 1, 0, 0, -(stereo_cx - max_disp), 0, 1, 0,
       -stereo_cy, 0, 0, 0, stereo_focal_px, 0, 0, (-1 / T[0]), 0);

  cv::fisheye::initUndistortRectifyMap(K1, D1, R_left, P_left, stereo_size,
                                       CV_32FC1, lmapx_, lmapy_);
  cv::fisheye::initUndistortRectifyMap(K2, D2, R_right, P_right, stereo_size,
                                       CV_32FC1, rmapx_, rmapy_);
...
void t265Depth::elaborateImages(const std_msgs::Header &header_msg) {
  ...
  if (image_left_.rows > 0 && image_right_.rows > 0) {
    cv::remap(image_left_, undist_image_left_, lmapx_, lmapy_,
              cv::INTER_LINEAR);
    cv::remap(image_right_, undist_image_right_, rmapx_, rmapy_,
              cv::INTER_LINEAR);
    ...
    if (pub_disparity_.getNumSubscribers() > 0) {
      ...
      out_disparity_msg =
          cv_bridge::CvImage(std_msgs::Header(), "mono8", left_disp8u)
              .toImageMsg();
